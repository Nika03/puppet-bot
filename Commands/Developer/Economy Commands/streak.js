...
module.exports = {
	name: "streak",
	description: "Check the daily streak leaderboard.",
	...
	async execute(interaction) {
		...
		if (is_blacklisted !== null) {
			if (!is_blacklisted.commands.includes(`streak`)) {
		...
		if (!userData) {
			await EconomyChecker.create({ user: interaction.user.id, daily_streak: 0 });
		}
		await interaction.deferReply();
		var u = await EconomyChecker.find().sort("-daily_streak");
		const array = [];
		let x = 0;
		let yes = false
		dontAdd = false;
		u.forEach((u) => {
			if (dontAdd == false) {
				array.push(
					`#${x + 1} | <@!${u.user}> with **${u.daily_streak}** days streak.`
				);
				x++;
			}
			if (x < 10){
		...
		if (yes == false) {
			newEmbed.addFields({
				name: "Your Position",
				value: ` #${userPlace} | ${interaction.user} with **${userData.daily_streak}** days streak.`,
			})
		}
