...
module.exports = {
	...
	async execute(interaction, client) {
		...
		if (interaction.guild.id === "946518364216520774") { // puppet server
		...
		} if(interaction.guild.id === "752104036102176778") { // nika server
			if (!roles.has("1042973536668631101") && !roles.has("1071605420218650714")) { // custom role pass and lvl 50 role
				return interaction.reply({
					content:
						"You cannot use this command without <@&1042973536668631101> or <@&752589981079437352>.",
					ephemeral: true,
				});
			}

			if (!roles.has("1042973536668631101")) // whalecum
				role_create_ding = 1;
			role_create_ding = 0;

			if (!roles.has("752589981079437352")) // default
				role_create_dong = 1;
			role_create_dong = 0;
			...
			if (role_create_ding + role_create_dong === 2) {
				return interaction.reply({
					content:
						"You cannot use this command without <@&1042973536668631101> and <@&752589981079437352>.",
					ephemeral: true,
				});
			}
			...
			roles.forEach((r) => {
				...
				role_position = guild.roles.cache.get("834456314662092880").position + 1; // black role
			if (name) {
				if (name.length > 100 - 9) {
					interaction.reply({
						content: "The name for your custom role is too big.",
