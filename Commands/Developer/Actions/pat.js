	...
	async execute(interaction, client) {
		...
		if (user.id === interaction.user.id) {
			return interaction.reply({
				embeds: [new MessageEmbed().setDescription(`You cannot pat yourself!`)],
				ephemeral: true,
			});
		}
			...
			return interaction.reply({
				embeds: [
					new MessageEmbed()
						...
						.setDescription(
							`You cannot pat a bot! Have a floppa pillow instead.`
						)
						.setImage(floppa)
		...
		// add to array "*pat pat*"
		const q = "pat_anime";

		try {
			tenor(q, interaction, MessageEmbed, "pets", interaction.member.user, user, " Adorable~");
		} catch (err) {
			console.log(err);
