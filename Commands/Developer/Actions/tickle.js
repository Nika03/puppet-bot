	...
	async execute(interaction, client) {
		...
		if (user.id === interaction.user.id) {
			return interaction.reply({
				embeds: [new MessageEmbed().setDescription(`You cannot tickle yourself!`)],
				ephemeral: true,
			});
		}
			...
			return interaction.reply({
				embeds: [
					new MessageEmbed()
						...
						.setDescription(
							`You cannot tickle a bot! Have a floppa pillow instead.`
						)
						.setImage(floppa)
		...
		try {
			tenor(q, interaction, MessageEmbed, "tickles", interaction.member.user, user, " Ha!");
		} catch (err) {
			console.log(err);
