	...
	async execute(interaction, client) {
		...
		if (user.id === interaction.user.id) {
			return interaction.reply({
				embeds: [new MessageEmbed().setDescription(`You cannot hold yourself!`)],
				ephemeral: true,
			});
		}
			...
			return interaction.reply({
				embeds: [
					new MessageEmbed()
						...
						.setDescription(
							`You cannot hold a bot! Have a floppa pillow instead.`
						)
						.setImage(floppa)
		...
		try {
			tenor(q, interaction, MessageEmbed, "grabs", interaction.member.user, user, " hands~ awww");
		} catch (err) {
			console.log(err);
