	...
	async execute(message, client) {
		...
		do {
			...
			if (array[ax] === filter[filterX]) {
				...
				channel.send({
					...
					embeds: [
						new MessageEmbed()
							...
							.setDescription(`\`${message.author.tag}\` (${message.author.id}) has been banned for triggering the filter.\n\`\`\`${message.content}\`\`\``)
							.setColor("DARK_NAVY")
							.setTimestamp(),
					],
				});
				...
				try {
					member.send({
						content: "discord.gg/neco-arc",
						embeds: [
							new MessageEmbed()
								.setColor("DARK_NAVY")
								.setDescription(
									`You have been banned in **Neco Puppeteers' Cult** for: \`Triggering the filter\`. You have been banned for **1 day** and this ban cannot be appealed.`
								),
						],
					});
				} catch (e) {
				...
				const banOptions = {
					reason: `has been banned for triggering the filter.Content: ${message.content}`,
				};
				setTimeout(() => {
					guild.members.ban(message.author.id, banOptions).catch((e) => {
						console.log(e);
					});
				}, 1000);
				stopf = true;
